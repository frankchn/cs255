...
function AES_Encrypt_String(key, plaintext) {
  ...
  for(var i = 0; i < num_blocks; i++) {
    ...
    var stream = cipher.encrypt(data);
    var pt_block = AES_Extract_Block(plaintext.substring(i * 16, (i + 1) * 16));

    for(var j = 0; j < 4; j++) {
      stream[j] ^= pt_block[j];
    }

    ciphertext += AES_Reconstruct_String(stream);
  }
...
function AES_Decrypt_String(key, nonceciphertext) {
  ...
  for(var i = 0; i < num_blocks; i++) {
    ...
    var stream = cipher.encrypt(data);
    var pt_block = AES_Extract_Block(ciphertext.substring(i * 16, (i + 1) * 16));
...
// By Chris Veness - http://www.movable-type.co.uk/scripts/sha1.html
/////////////////////////////////////////////////////////
